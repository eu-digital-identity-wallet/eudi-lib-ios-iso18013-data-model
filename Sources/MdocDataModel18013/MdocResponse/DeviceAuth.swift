...
extension DeviceAuth: CBORDecodable {
	public init(cbor: CBOR) throws(MdocValidationError) {
		guard case let .map(m) = cbor else { throw .deviceAuthInvalidCbor }
		if let cs = m[Keys.deviceSignature] {
			if let ds = Cose(type: .sign1, cbor: cs) { coseMacOrSignature = ds } else { throw .deviceAuthInvalidCbor }
		} else if let cm = m[Keys.deviceMac] {
			if let dm = Cose(type: .mac0, cbor: cm) { coseMacOrSignature = dm } else { throw .deviceAuthInvalidCbor }
		} else { throw .deviceAuthInvalidCbor }
	}
}
