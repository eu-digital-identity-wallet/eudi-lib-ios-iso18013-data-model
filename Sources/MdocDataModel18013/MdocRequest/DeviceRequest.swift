...
extension DeviceRequest: CBORDecodable {
    public init(cbor: CBOR) throws(MdocValidationError) {
        ...
        version = v
		if v.count == 0 || v.prefix(1) != "1" { throw .deviceRequestInvalidCbor }
        guard case let .array(cdrs) = m[Keys.docRequests] else { throw .deviceRequestMissingField(Keys.docRequests.rawValue) }
        do { docRequests = try cdrs.map { try DocRequest(cbor: $0) } } catch { throw .deviceRequestInvalidCbor }
        guard docRequests.count > 0 else { throw .deviceRequestInvalidCbor }
    }
}
